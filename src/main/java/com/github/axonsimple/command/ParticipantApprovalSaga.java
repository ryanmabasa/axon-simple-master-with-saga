...
public class ParticipantApprovalSaga {
    ...
    @SagaEventHandler(associationProperty = "roomId")
    public void handle(PendingEvent event) {
        logger.info("[Saga] Handle pending {}", event);
        SagaLifecycle.associateWith("shipmentId", "5");
        commandGateway.send(new ShipmentCommand("5"));
    }

    @SagaEventHandler(associationProperty = "shipmentId")
    public void handle(ShipmentCreatedEvent event) {
        logger.info("[Saga] Handle sssssss {}", event);
